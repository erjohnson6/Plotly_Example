...
Europe.Confirmed$Country.Region <- factor(Europe.Confirmed$Country.Region)
Europe.Confirmed$Province.State <- factor(Europe.Confirmed$Province.State)

```
For the sake of brevity we'll only look at the first few lines:

``` {r Head}
head(Europe.Confirmed)
```
...
``` {r graf}
plot_ly(data = Europe.Confirmed, x = ~Date, y = ~Confirmed, 
